...
<template>
  ...
  <template v-else-if="detail">
    <section class="catalog-overview surface-card">
      <div><span class="help-label">상태 <HelpTooltip label="카탈로그 상태" :text="HELP_TOOLTIP_COPY.catalogStatus" /></span><strong><i class="status-dot" :class="{ active: detail.enabled }" />{{ detail.enabled ? '활성' : '비활성' }}</strong></div>
      <div><span class="help-label">Catalog key <HelpTooltip label="Catalog key" :text="HELP_TOOLTIP_COPY.catalogKey" /></span><strong>{{ detail.catalogKey }}</strong></div>
      <div><span class="help-label">Revision <HelpTooltip label="Revision" :text="HELP_TOOLTIP_COPY.revision" /></span><strong>{{ detail.revision }}</strong></div>
      <div><span class="help-label">Schema <HelpTooltip label="Schema" :text="HELP_TOOLTIP_COPY.schema" /></span><strong>{{ detail.schemaVersion }}</strong></div>
    </section>
