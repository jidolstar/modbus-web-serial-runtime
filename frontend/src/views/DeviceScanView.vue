...
<template>
  <header class="page-heading">
    <div><p class="eyebrow">DEVICE DISCOVERY</p><h1>장비 스캔</h1><p class="description">표준 8N1 조합에서 응답하는 Modbus 장비의 Baudrate와 Slave ID를 찾습니다.</p></div>
  ...
  <section class="surface-card scan-config-panel">
    <div class="section-heading"><div><h2>스캔 범위</h2><p>범위를 좁힐수록 장비 탐색이 빠르게 끝납니다.</p></div></div>
    <fieldset class="scan-baud-options" :disabled="scanning">
      <legend>Baudrate</legend>
    ...
    <div class="scan-range-fields">
      <label>시작 Slave ID<input v-model.number="slaveIdStart" type="number" min="1" max="247" inputmode="numeric"></label>
      <label>종료 Slave ID<input v-model.number="slaveIdEnd" type="number" min="1" max="247" inputmode="numeric"></label>
    </div>
  ...
  <section class="surface-card scan-results-panel">
    <div class="section-heading"><div><h2>스캔 결과</h2><p>응답한 통신 조합만 표시하며 장비 모델을 자동으로 추정하지 않습니다.</p></div><span>{{ results.length }}건</span></div>
    <div v-if="results.length" class="scan-result-list">
      <article v-for="result in results" :key="`${result.serialConfig.baudRate}:${result.serialConfig.dataBits}:${result.serialConfig.parity}:${result.serialConfig.stopBits}:${result.serialConfig.flowControl}:${result.slaveId}`" class="scan-result-row">
        ...
        <span class="status-pill" :class="statusClass(result.status)">{{ statusLabel(result.status) }}</span>
