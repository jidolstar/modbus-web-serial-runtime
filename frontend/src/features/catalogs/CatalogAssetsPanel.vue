...
<template>
  <section v-if="mode === 'files'" class="asset-section surface-card">
    <div class="section-heading"><div><h3 class="help-label">참고 파일 <HelpTooltip label="참고 파일" :text="HELP_TOOLTIP_COPY.referenceFile" /></h3><p class="help-label">검증 상태가 clean인 자료만 다운로드할 수 있습니다. <HelpTooltip label="clean 상태" :text="HELP_TOOLTIP_COPY.cleanStatus" /></p></div></div>
    <div class="asset-form three-columns"><label>제목<input v-model="fileTitle" maxlength="160" placeholder="RS485 통신 프로토콜"></label><div class="form-field"><span class="field-label"><label for="catalog-document-type">문서 유형</label><HelpTooltip label="문서 유형" :text="HELP_TOOLTIP_COPY.documentType" /></span><select id="catalog-document-type" v-model="documentType"><option value="communication_protocol">통신 프로토콜</option><option value="manual">매뉴얼</option><option value="datasheet">데이터시트</option><option value="reference_image">참고 이미지</option><option value="other">기타</option></select></div><label>파일<input type="file" accept=".pdf,.txt,.png,.jpg,.jpeg,.webp,.doc,.docx" @change="chooseFile"></label><button class="button button-primary" type="button" :disabled="busy || !selectedFile || !fileTitle.trim()" @click="uploadFile">파일 등록</button></div>
    <ul v-if="files.length" class="asset-list"><li v-for="file in files" :key="file.id"><div><strong>{{ file.title }}</strong><span>{{ file.originalName }} · {{ Math.ceil(file.byteSize / 1024) }}KB</span></div><span class="file-status-with-help"><span class="status-pill" :class="file.status === 'clean' ? 'success' : 'warning'">{{ file.status }}</span><HelpTooltip label="파일 검사 상태" :text="HELP_TOOLTIP_COPY.cleanStatus" /></span><div class="inline-actions"><button class="button button-ghost button-compact" type="button" :disabled="file.status !== 'clean'" @click="download(file)">다운로드</button><button class="button danger button-compact" type="button" @click="removeFile(file)">삭제</button></div></li></ul><p v-else class="empty-copy">등록된 참고 파일이 없습니다.</p>
  </section>
  <section v-else class="asset-section surface-card">
    <div class="section-heading"><div><h3 class="help-label">참고 링크 <HelpTooltip label="참고 링크" :text="HELP_TOOLTIP_COPY.referenceLink" /></h3><p>서버는 주소를 방문하지 않고 HTTPS URL만 보관합니다.</p></div></div>
    <div class="asset-form three-columns"><label>제목<input v-model="linkTitle" maxlength="160" placeholder="제조사 제품 페이지"></label><div class="form-field"><span class="field-label"><label for="catalog-link-type">링크 유형</label><HelpTooltip label="링크 유형" :text="HELP_TOOLTIP_COPY.linkType" /></span><select id="catalog-link-type" v-model="linkType"><option value="official_website">공식 홈페이지</option><option value="manufacturer_page">제조사 페이지</option><option value="documentation">문서</option><option value="reference">참고 자료</option><option value="retailer">판매처(쇼핑몰·판매점)</option></select></div><div class="form-field"><span class="field-label"><label for="catalog-link-url">HTTPS URL</label><HelpTooltip label="HTTPS URL" :text="HELP_TOOLTIP_COPY.httpsUrl" /></span><input id="catalog-link-url" v-model="linkUrl" type="url" maxlength="2048" placeholder="https://example.com/products/temp-100"></div><div class="inline-actions"><button class="button button-primary" type="button" :disabled="busy || !linkTitle.trim() || !linkUrl.trim()" @click="saveLink">{{ editingLinkId ? '링크 수정' : '링크 추가' }}</button><button v-if="editingLinkId" class="button button-ghost" type="button" @click="clearLinkForm">취소</button></div></div>
